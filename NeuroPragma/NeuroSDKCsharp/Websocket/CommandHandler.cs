...
public sealed class CommandHandler
{
    ...
    public void Handle(string command, IncomingData data)
    {
        ...
        try
        {
            foreach (var handler in _handlers.OfType<IIncomingMessageHandler>().Where(handler => handler.CanHandle(command)))
            {
                ...
                catch (Exception e)
                {
                    ...
                    REFrameworkNET.API.LogError($"Error in command handling: {e}");
                    resultData = null;  
                }

                if (!validationResult.Successful)
                {
                    REFrameworkNET.API.LogWarning("Unsuccessful execution result when handling message");
                }
                ...
                if (validationResult.Successful)
                {
                    REFrameworkNET.API.LogInfo($"CommandHandler validation successful");
        ...
        {
            REFrameworkNET.API.LogError($"Issue in Handle\n  {e}");
            throw;
        }
