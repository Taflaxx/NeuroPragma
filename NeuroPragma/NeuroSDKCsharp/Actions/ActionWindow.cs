...
public sealed class ActionWindow
{
    ...
    public static ActionWindow Create()
    {
        ...
        catch (Exception e)
        {
            ...
            REFrameworkNET.API.LogError($"Issue with creating action window: {e}");
            throw;
        }
    }
    ...
    public void Initialize()
    {
        ...
        {
            REFrameworkNET.API.LogError($"ActionWindow was not created correctly, you should use the Create method");
        }
    }
    ...
    private bool ValidateFrozen()
    {
        ...
        {
            REFrameworkNET.API.LogError("Cannot change action window after it has been registered");
            return false;
        }
    ...
    public void Register()
    {
        if (CurrentState != State.Building)
        {
            REFrameworkNET.API.LogError("Cannot register action window more than once");
            return;
        }

        if (_actions.Count == 0)
        {
            REFrameworkNET.API.LogError("Cannot register an action window with no actions");
            return;
        }
    ...
    public ActionWindow AddAction(INeuroAction action)
    {
        ...
        if (action.ActionWindow != null)
        {
            ...
            {
                REFrameworkNET.API.LogError($"Cannot add action {action.Name} to this action window as it is already in another action window");
            }
        ...
        {
            REFrameworkNET.API.LogError($"Cannot add two action that have the same name. The name was {action.Name}");
            return this;
        }
    ...
    public void End()
    {
        ...
        REFrameworkNET.API.LogInfo($"Ending action window");
